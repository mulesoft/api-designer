  ...
  angular.module('fs', [
    ...
    function ($q, $window, $rootScope, $timeout, ramlRepositoryConfig, ramlRepositoryElements, ramlSnippets, fileSystem) {
      ...
      service.exportFiles = function exportFiles(zipName) {
        ...
        function exportDirectory(directory, jszip) {
          ...
          for (var i = 0; i < directory.children.length; i++) {
            ...
            } else if (child.name.slice(-5) !== '.meta') {
              // Skip meta files
      ...
      service.loadAndUpdateDirectory = function loadAndUpdateDirectory() {
        return fileSystem.directory(BASE_PATH).then(service.updateDirectory.bind(this));
      };
      service.updateDirectory = function updateDirectory(rootChildren) {
        ...
        var addElement = function (element) {
          if (!!service.getByPath(element.path)) {
            return;
          }
      ...
      service.saveAndUpdate = function saveAndUpdate(files, file, update) {
        if (update) {
          return service.saveAndUpdateRoot(files).then(service.getByPath.bind(this, file.path)).then(service.loadFile.bind(this));
        } else {
